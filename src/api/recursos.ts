...
export interface EnvioRemisionResumen {
  ...
  fecha: string;
  total: number;
  cliente: { nombre: string; ciudad: string | null; telefono: string | null; whatsapp: string | null };
  sede: { nombre: string };
}
...
export const skydropxApi = {
  ...
  sincronizar: () =>
    api<{ total: number; creados: number; actualizados: number }>("/skydropx/sincronizar", { method: "POST" }),
};
